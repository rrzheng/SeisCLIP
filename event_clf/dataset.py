...
class PNWDataset(Dataset):
    def __init__(self,
        ...
        self._info_size = label_size
        assert chunk_size == (spec_size + label_size)
    ...
    def _read_chunk_from_memmap(self, path: str, index: int) -> Tuple[np.ndarray, int]:
        item_size = self.dtype(0).itemsize
        ...
        label = array[self._spec_size:]
        return spec.copy(), int(label[0])
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, int]:
        ...
        spec, label = self._read_chunk_from_memmap(self._paths[memmap_index], memmap_local_index)
        spec = self._z_norm(spec)
        spec = self._cal_norm_spectrogram(spec)
        return torch.tensor(spec, dtype=torch.float64), label
