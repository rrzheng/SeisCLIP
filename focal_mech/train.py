...
from model_v2 import build_model_from_pretrain
...
def parse_args():
    ...
    parser.add_argument("--input_dir", type=str, default='focal_mech',
    ...
    parser.add_argument('--model_path', type=str, default='./focal_mech/',
...
def evaluate(data_loader, model, criterion, device):
    ...
    with torch.no_grad():
        model.eval()
        for mask, spec, stat_info, label in data_loader:
            mask = mask.to(device)
            spec = spec.to(device)
...
def main(args):
    ...
    train_dataset = MechDataset(*train_path)
    val_dataset = MechDataset(*val_path)
