...
class MechDataset(Dataset):
    ...
    def _read_chunk_from_memmap(self, path: str, index: int) -> Tuple[int, np.ndarray, np.ndarray, int]:
        item_size = self.dtype(0).itemsize
    ...
    def __getitem__(self, index: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray, int]:
        ...
        seq_len, spec, stat_info, label = self._read_chunk_from_memmap(self._paths[memmap_index], memmap_local_index)
